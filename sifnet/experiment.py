...
# from sifnet.medium_term_ice_forecasting.utilities.visualize_correlation import evaluate_freezeup_breakup_at_region, \
#     evaluate_freezeup_breakup_at_ports
...
class Experiment:
    ...
    # def run_k_fold(self, folds, model_key, training_kwargs, model_kwargs, generate_graphs=True, **kwargs):
    #     """
    #     Run k fold cross validation. Data set will be divided into num_years/folds sections. The ith year of every
    #     section will be part of the test set of the ith fold.

    #     Generates a directory containing subdirectories for each fold. Each subdirectory has the same files generated
    #     from experiment.run. The parent directory contains aggregated data cube

    #     Plots that are created include:
    #         - overall model performance at different lead days compared to persistence and climate normal performance
    ...
    #     savepath = os.path.join(self.current_path, model_directory)
    #     test_record = dict()
    #     for i in range(folds):
    #         test_years = years[i::folds]
    #         test_record[i] = test_years

    #         self.dataset_manager.config(days_of_historic_input=historic_days, forecast_days_forward=forecast_days,
    #                                     validation_years=[], test_years=test_years)

    #         fold_directory = "fold_" + str(i)
    ...
    #         # i.e. Hudson Bay, NWT
    #         evaluate_freezeup_breakup_at_region(savepath, landmask,
    #                                             self.dataset_manager.forecast_days_forward,
    #                                             self.dataset_manager.days_of_historic_input,
    #                                             region_name=region,
    #                                             lat_bounds=lat_bounds,
    #                                             long_bounds=long_bounds,
    #                                             season=self.find_season(),
    #                                             load=True)

    #         # plot qqplot of predicted vs observed freeze-up/breakup date at a specific location, usually
    #         if self.find_region() == 'Hudson Bay':
    #             evaluate_freezeup_breakup_at_ports(savepath, landmask,
    #                                                self.dataset_manager.forecast_days_forward,
    #                                                self.dataset_manager.days_of_historic_input,
    #                                                [29], 'hudson_ports.yaml',
    #                                                self.dataset_manager.resolution,
    #                                                lat_bounds=lat_bounds,
    #                                                long_bounds=long_bounds,
    #                                                season=self.find_season())
